...
namespace t4tea.repository.Repositories
{
    public class GenericRepository<TEntity> : IGenericRepository<TEntity> where TEntity : BaseEntity
    {
        ...
        {
            return await _context.Set<Products>().Include(y => y.images).Include(z => z.benifits).Include(x => x.reviews).ToListAsync();
        }

        
        public async Task<Products> GetProductByIdAsync(int id)
        {
            return await _context.Set<Products>().Include(y => y.images).Include(z => z.benifits).Include(x => x.reviews).FirstOrDefaultAsync(x=>x.Id == id);
        }
